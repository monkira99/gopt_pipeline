...
def extract_split_features(split_name, ds_split, out_path,
    ...
    for batch in loader:
        ...
        if use_fp16 and device.startswith("cuda"):
            iv = iv.half()
        k_mask = k_inp.attention_mask.to(device, dtype=torch.long) if getattr(k_inp, "attention_mask", None) is not None else None
        ...
        if use_wavlm and wl_model is not None:
            ...
            if use_fp16 and device.startswith("cuda"):
                wf = wf.half()
            w_mask = w_inp.attention_mask.to(device, dtype=torch.long) if getattr(w_inp, "attention_mask", None) is not None else None
